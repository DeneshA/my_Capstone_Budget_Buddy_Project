...
class Expense(models.Model):
    PAY_TERMS = [('Daily', 'Daily'), ('Weekly', 'Weekly'), ('Bi-Weekly', 'Bi-Weekly'), ('Monthly', 'Monthly'), ('Annually', 'Annually')]
    DURATION_TERM = [('Month', 'Month'), ('Year', 'Year')]
    user_id = models.ForeignKey(User, on_delete=models.CASCADE, related_name='expense')  # using Django's built-in User model
    category_id = models.ForeignKey(Category, on_delete=models.CASCADE, related_name='expense')
    duration = models.IntegerField()
    duration_terms = models.CharField(max_length=20, choices=DURATION_TERM)
    start_date = models.DateField(null=True,blank=True)
    end_date = models.DateField(null=True,blank=True)
